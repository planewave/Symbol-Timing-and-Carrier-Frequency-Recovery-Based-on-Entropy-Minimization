% paper figure: Performance of two symbol timing algorithms in the presence of CFO
% 大论文 性能仿真 NDA  Entropy 算法 BPSK QAM 频偏
% 红皮书 p404 比较
clear
close all
clc
...
data = randi([0 1],k*num_trial+300,1);
data2 = randi([0 1],2*num_trial+600,1);
hMod2 = comm.QPSKModulator('BitInput',true);
hMod = comm.PSKModulator('BitInput',true,'ModulationOrder',2,'PhaseOffset',pi/M);
...
txSig =  txSig .*exp(1j*0.001*pi*t');
txSig2 = txSig2.*exp(1j*0.001*pi*t');
%%
r = [0.55:-0.05:0.09]; %熵距离参数
% r = 0.5:-0.04:0.01;
% r = 0.3*ones(1,9);
for p = 1:length(EsN0)
%     SNR = 40 - 10*log10(sps);
...
%         entp2 = zeros(sps,1);
        if gpu
            ...
            tau_entrp = gpuArray(single(tau_entrp));
        end
        entp2 = entp;
        for m = 1:sps
            entp(m) = simpEntpEng(txL0_rs(m,:),r(p),0.3, gpu);
            entp2(m) = simpEntpEng(txL0_rs2(m,:),r(p),0.3, gpu);
        end
...
legend('BPSK, EM','BPSK, O&M','QPSK, EM','QPSK, O&M','MCRB')
xlabel('E_s/N_0 (dB)')
